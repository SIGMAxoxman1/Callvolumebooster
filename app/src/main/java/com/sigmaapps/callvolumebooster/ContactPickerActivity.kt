...
class ContactPickerActivity : AppCompatActivity() {
    ...
    data class DeviceContact(val contactId: String, val name: String, val number: String)
    ...
    /**
     * Stars (or unstars) the contact in the device's own contacts database.
     * This is what actually makes Android's Do Not Disturb "allow calls
     * from starred contacts" policy apply to this specific person — it's
     * not something our app can fake from the outside.
     */
    private fun setStarred(contactId: String, starred: Boolean) {
        try {
            ...
            Toast.makeText(
                this,
                "Couldn't star this contact — check contacts permission.",
                Toast.LENGTH_SHORT
            ).show()
        }
    }
            ...
            while (it.moveToNext()) {
                ...
                if (contactId != null && name != null && number != null) {
                    ...
                    if (normalized.isNotEmpty() && seen.add(normalized)) {
                        result.add(DeviceContact(contactId, name, number))
                    }
                }
            }
        }
